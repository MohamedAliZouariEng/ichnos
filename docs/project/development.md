...
## Traces and answers in tests

`ichnos/trace/` builds traces from synced rows without a model, validates them, and reads confirmations. `ichnos/answers/` retrieves numbered sources and enforces grounded answers. `tests/test_trace.py` seeds one BRD, Stories, pull requests, commits, a changed test file and a check run, and the trace, validation, confirmation, retrieval and answer tests reuse that fixture. `tests/test_question_set.py` holds the fixed question set: the sources each question must retrieve, the gaps it must or must not have, retrieval recall of at least 90%, and scripted models that invent citations or claim untested behaviour is tested, to prove enforcement removes both.
